...
## 未来计划预告 (Announcement of Next-up)
(Identify the information of new products or projects to be up in future. For each topic, summarize the viewpoints expressed by different speakers (using their identified names or A, B, C markers). Highlight agreements and disagreements where applicable. Only mention speakers who actively contributed to the discussion on that specific topic. Use Simplified Chinese.)
**[预告1]**:
...
**[预告2]**:
...
...
Identify Speakers and Format: Analyze the transcript content for speaker introductions (self-introductions or introductions by others). Whenever a speaker's name is clearly identifiable, use their actual name (e.g., "张三:", "John Doe:") as the speaker marker. Only use generic markers (说话人1:, 说话人2:, 说话人3:, etc.) for speakers whose names cannot be confidently determined from the dialogue. Format the polished dialogue using these identified or generic speaker markers.
...
Please ensure the output is in Simplified Chinese for the main sections (Summary, Key Takeaways, New Product or Project Update, Announcement of Next-up) as specified, and the Detailed Transcript is either English (if original was English) or Simplified Chinese (otherwise). Use Markdown formatting throughout, and accurately reflect the content of the transcript while incorporating the requested polishing and formatting for the Detailed Transcript section.
```
...
## 摘要 (Summary)
(Provide a brief narrative paragraph summarizing the main points and outcomes of the meeting in Simplified Chinese. Mention背景、核心观点、时间/地域范围与结论基调。)

## 关键要点 (Key Takeaways)
(List the core conclusions, decisions, risks, opportunities, or action recommendations in bullet form. Each item must include the观点本身以及支撑信息，如关键数据、发言人引用或示例。请确保把逐字稿中提及的要点全部覆盖。)
**要点 1**
   * 要点 1 明细
**要点 2**
   * 要点 2 明细
...
## 关键议题 (Key Topics)
(Identify every major topic discussed. For each topic, add a level-3 heading `### [议题名称]`，并在段落内描述不同发言人的观点与立场，注明共识、分歧以及引用的理由或证据。仅列出实际讨论该议题的发言人。)
### [议题名称]:
...
### [议题名称]:
...
